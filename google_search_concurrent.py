                ...
                for future in in_process:
                    if future.done():
                        ...
                        if len(result) > 0:
                            ...
                            else: credibility = 'Third-Party Source'

                            response.append({'source':ut.extract_domain(url), 'url':url, 'credibility': credibility, 'text':result})
...
def response_text_extract(query_phrase, keywords, keyword_weights, url, response, get_time):
    ...
    if prefix_text.startswith('query:'):
        ...
        if text_index > 0:
            ...
            prefix_text = response_text[:min(len(response_text), 96)].lower()
    if ('no information' in prefix_text or 'i cannot provide' in prefix_text
        or 'as an ai language model' in prefix_text or 'does not provide' in prefix_text
        or 'it is not possible' in prefix_text 
        ):
        # skip this summary, no info
        print("{} {}/{}/{}/{}".format(site, len(response), len(url_text), len(extract_text),0))
        #print('************')
        #print(extract_text)
        #print('************')
        sys.stdout.flush()
    ...
    if (prefix_text.startswith("i'm sorry") or prefix_text.startswith("there is no ")
        or (prefix_text.startswith("the provided text") or prefix_text.startswith('i cannot')
            or prefix_text.startswith("unfortunately")
            or prefix_text.startswith("sorry")
            or prefix_text.startswith("the text"))
        and ('is not relevant' in prefix_text or 'no information' in prefix_text or 'does not provide' in prefix_text
             or 'does not contain' in prefix_text or 'no relevant information' in prefix_text)):
        # skip this summary, no info
        ...
        for sentence in sentences:
            if ('no inform' in sentence.lower() or 'no specific inform' in sentence.lower()
                ...
                or 'not mention' in sentence.lower() or 'not specifically mention' in sentence.lower()):
                pass
            else:
                response_text += '\n \u2022 '+sentence+'. '
